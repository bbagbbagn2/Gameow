...
export const signinSchema = z.object({
	email: z.string().email({ message: SIGNIN_ERRORS.INVALID_EMAIL }),
	password: z.string().min(8, { message: SIGNIN_ERRORS.TOO_SHORT_PASSWORD })
});
...
export const Step1Schema = z.object({
	name: z
		.string()
		.nonempty({ message: CREATE_GATHERING_ERRORS.EMPTY.NAME })
		.min(2, { message: CREATE_GATHERING_ERRORS.LIMIT.MIN.NAME })
		.max(20, { message: CREATE_GATHERING_ERRORS.LIMIT.MAX.NAME })
		.regex(
	...
	location: z.custom<GatheringLocation>(val => typeof val === 'string' && val.length > 0, {
		message: CREATE_GATHERING_ERRORS.EMPTY.LOCATION
	})
});

export const Step2Schema = z.object({
	type: z.custom<GatheringType>(val => typeof val === 'string' && val.length > 0, {
		message: CREATE_GATHERING_ERRORS.EMPTY.TYPE
	}),
	image: z
...
export const Step4Schema = z.object({
	capacity: z
		.number({ message: CREATE_GATHERING_ERRORS.EMPTY.CAPACITY })
		.min(5, { message: CREATE_GATHERING_ERRORS.LIMIT.MIN.CAPACITY })
		.max(20, { message: CREATE_GATHERING_ERRORS.LIMIT.MAX.CAPACITY })
});
...
export const profileEditSchema = z.object({
	companyName: z
		.string()
		.trim()
		.min(2, { message: '닉네임은 2글자 이상 적어주세요.' })
